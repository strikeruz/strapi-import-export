...
exports.exportDataV3 = exportDataV3;
const models_1 = require("../../utils/models");
const constants_js_1 = require("../../config/constants.js");
const query_1 = require("../../utils/query");
const export_context_1 = require("./utils/export-context");
const export_processor_1 = require("./utils/export-processor");
async function exportDataV3({ slug, search, applySearch, exportPluginsContentTypes, documentIds, maxDepth = 20, exportAllLocales = true, exportRelations = false, deepPopulateRelations = false, deepPopulateComponentRelations = false, }) {
    const slugsToExport = slug === constants_js_1.CustomSlugs.WHOLE_DB
        ? (0, models_1.getAllSlugs)({ includePluginsContentTypes: exportPluginsContentTypes })
        : [constants_js_1.CustomSlugToSlug[slug] || slug];
    const searchParams = applySearch ? (0, query_1.buildFilterQuery)(search) : {};
    const context = new export_context_1.ExportContext({
        documentIds,
        applySearch,
        search: searchParams,
        exportAllLocales,
    ...
    const processor = new export_processor_1.ExportProcessor(context, {
    ...
    let loopCount = 0;
    while (Object.keys(context.getRelations()).length > 0 &&
        exportRelations &&
        loopCount < maxDepth) {
...
//   if (model.pluginOptions?.[pluginId]?.idField) {
