...
export function attributeIsUnique(
  attribute: Schema.Attribute.AnyAttribute
): attribute is Schema.Attribute.OfType<
  'string' | 'text' | 'email' | 'integer' | 'biginteger' | 'float' | 'decimal'
> {
  return ['string', 'text', 'email', 'integer', 'biginteger', 'float', 'decimal'].includes(
    attribute.type
  );
}
